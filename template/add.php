...
<a rel="facebox" href="addcourse.php"><Button type="submit" class="btn btn-info" style="float:right; width:230px; height:35px;" /><i class="icon-plus-sign icon-large"></i> Add course </button></a><br><br>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			...
			<th width="15%"> Tutorial </th>
			<th width="15%"> Labsheet </th>
			<th width="15%"> Note </th>
	...
	<tbody>
			...
			<?php
				...
				include('../connect.php');
				$result = $db->prepare("SELECT * FROM course ORDER BY courseid DESC");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
			?>
			<tr class="record">
			<td><?php echo $row['cname']; ?></td>
			<td><a href="images/<?php echo $row['clecture']; ?>"><?php echo $row['clecture']; ?> </a></td>
			<td><?php echo $row['ctute']; ?></td>
			<td><?php echo $row['clab']; ?></td>
			<td><?php echo $row['cnote']; ?></td>


			<td><a  title="Click To Edit Stock Report" rel="facebox" href="edit.php?id=<?php echo $row['courseid']; ?>"><button class="btn btn-warning btn-mini"><i class="icon-edit"></i> Upload </button></a> 
			<a href="#" id="<?php echo $row['courseid']; ?>" class="delbutton" title="Click To Delete"><button class="btn btn-danger btn-mini"><i class="icon-trash"></i> Delete</button></a></td>
			</tr>
			<?php
				}
